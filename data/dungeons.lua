local Deathless = Deathless

Deathless.Data = Deathless.Data or {}

Deathless.Data.Dungeons = {
    {
        id = "ragefire_chasm",
        name = "Ragefire Chasm",
        zone = "Orgrimmar",
        endBoss = "Bazzalan",
        bossLevel = 16,
        levelMin = 13,
        levelMax = 18,
        warnings = {
            "[[Taragaman the Hungerer]] casts a knockback - do not stand next to the lava.",
            "[[Taragaman the Hungerer]] casts a Fire Nova AoE.",
        },
        quests = {
            {
                name = "Hidden Enemies",
                questId = 5728,
                level = 16,
                side = "Horde",
                prereq = true,
                startNpc = "Thrall", startNpcId = 4949, startLoc = "Orgrimmar", startCoords = "32.0, 37.8",
                rewards = {
                    { itemId = 15443, name = "Kris of Orgrimmar" },
                    { itemId = 15445, name = "Hammer of Orgrimmar" },
                    { itemId = 15424, name = "Axe of Orgrimmar" },
                    { itemId = 15444, name = "Staff of Orgrimmar" },
                },
            },
            {
                name = "Searching for the Lost Satchel",
                questId = 5722,
                level = 15,
                side = "Horde",
                prereq = false,
                startNpc = "Rahauro", startNpcId = 5769, startLoc = "Thunder Bluff", startCoords = "70.6, 29.8",
                money = 700,
            },
            {
                name = "Slaying the Beast",
                questId = 5761,
                level = 16,
                side = "Horde",
                prereq = false,
                startNpc = "Neeru Fireblade", startNpcId = 3216, startLoc = "Orgrimmar", startCoords = "49.6, 50.6",
                money = 800,
            },
            {
                name = "Testing an Enemy's Strength",
                questId = 5723,
                level = 15,
                side = "Horde",
                prereq = false,
                startNpc = "Rahauro", startNpcId = 5769, startLoc = "Thunder Bluff", startCoords = "70.6, 29.8",
                money = 700,
            },
            {
                name = "The Power to Destroy...",
                questId = 5725,
                level = 16,
                side = "Horde",
                prereq = false,
                startNpc = "Varimathras", startNpcId = 2425, startLoc = "Undercity", startCoords = "56.2, 92.4",
                rewards = {
                    { itemId = 15449, name = "Ghastly Trousers" },
                    { itemId = 15450, name = "Dredgemire Leggings" },
                    { itemId = 15451, name = "Gargoyle Leggings" },
                },
            },
        },
    },
    {
        id = "wailing_caverns",
        name = "Wailing Caverns",
        zone = "The Barrens",
        endBoss = "Mutanus the Devourer",
        bossLevel = 22,
        levelMin = 15,
        levelMax = 25,
        warnings = {
            "Deviate Guardian will call nearby raptors when their health is low.",
            "Deviate Dreadfang casts a fear effect.",
            "[[Verdan the Everliving]] hits the tank for very high melee damage.",
        },
        quests = {
            {
                name = "Deviate Eradication",
                questId = 1487,
                level = 21,
                side = "Both",
                prereq = false,
                startNpc = "Ebru",
                startNpcId = 5768,
                startLoc = "Skull cave above the Wailing Caverns",
                money = 2500,
                rewards = {
                    { itemId = 6476, name = "Pattern: Deviate Scale Belt" },
                    { itemId = 6481, name = "Dagmire Gauntlets" },
                    { itemId = 8071, name = "Sizzle Stick" },
                },
            },
            {
                name = "Deviate Hides",
                questId = 1486,
                level = 17,
                side = "Both",
                prereq = false,
                startNpc = "Nalpak",
                startNpcId = 5767,
                startLoc = "Skull cave above the Wailing Caverns",
                money = 1800,
                rewards = {
                    { itemId = 918, name = "Deviate Hide Pack" },
                    { itemId = 6480, name = "Slick Deviate Leggings" },
                },
            },
            {
                name = "Leaders of the Fang",
                questId = 914,
                level = 22,
                side = "Horde",
                prereq = true,
                startNpc = "Nara Wildmane", startNpcId = 5770, startLoc = "Thunder Bluff", startCoords = "75.6 31.2",
                money = 0,
                rewards = {
                    { itemId = 6505, name = "Crescent Staff" },
                    { itemId = 6504, name = "Wingblade" },
                },
            },
            {
                name = "Serpentbloom",
                questId = 962,
                level = 18,
                side = "Horde",
                prereq = false,
                startNpc = "Apothecary Zamah", startNpcId = 3419, startLoc = "Thunder Bluff", startCoords = "23.6 21.4",
                money = 2000,
                rewards = {
                    { itemId = 10919, name = "Apothecary Gloves" },
                },
            },
            {
                name = "Smart Drinks",
                questId = 1491,
                level = 18,
                side = "Both",
                prereq = true,
                startNpc = "Mebok Mizzyrix", startNpcId = 3446, startLoc = "Ratchet", startCoords = "62.4 37.6",
                money = 1000,
            },
            {
                name = "The Glowing Shard",
                questId = 6981,
                level = 26,
                side = "Both",
                prereq = false,
                startNpc = "Glowing Shard",
                startItemId = 10441,
                startLoc = "Drops from Mutanus the Devourer",
                money = 0,
            },
            {
                name = "Trouble at the Docks",
                questId = 959,
                level = 18,
                side = "Both",
                prereq = false,
                startNpc = "Crane Operator Bigglefuzz", startNpcId = 3665, startLoc = "Ratchet", startCoords = "63.0 37.6",
                money = 1000,
            },
        },
    },
    {
        id = "deadmines",
        name = "The Deadmines",
        zone = "Westfall",
        endBoss = "Edwin VanCleef",
        bossLevel = 21,
        levelMin = 15,
        levelMax = 25,
        warnings = {
            "Patrols of Defias mobs roam the mine tunnels. Watch for them during pulls and after boss fights.",
            "Low-level players can accidentally body pull enemies on the upper levels of the pirate ship.",
            "[[Edwin VanCleef]] summons 2 adds at 50% and 25% HP. Burn them fast or they will overwhelm the healer.",
        },
        quests = {
            {
                name = "Collecting Memories",
                questId = 168,
                level = 18,
                side = "Alliance",
                prereq = false,
                startNpc = "Wilder Thistlenettle", startNpcId = 656, startLoc = "Stormwind City", startCoords = "65.2 21.6",
                rewards = {
                    { itemId = 2037, name = "Tunneler's Boots" },
                    { itemId = 2036, name = "Dusty Mining Gloves" },
                },
            },
            {
                name = "Oh Brother...",
                questId = 167,
                level = 20,
                side = "Alliance",
                prereq = false,
                startNpc = "Wilder Thistlenettle", startNpcId = 656, startLoc = "Stormwind City", startCoords = "65.2 21.6",
                rewards = {
                    { itemId = 1893, name = "Miner's Revenge" },
                },
            },
            {
                name = "Red Silk Bandanas",
                questId = 214,
                level = 17,
                side = "Alliance",
                prereq = true,
                startNpc = "Scout Riell", startNpcId = 820, startLoc = "Westfall", startCoords = "56.6 47.4",
                rewards = {
                    { itemId = 2074, name = "Solid Shortblade" },
                    { itemId = 2089, name = "Scrimshaw Dagger" },
                    { itemId = 6094, name = "Piercing Axe" },
                },
            },
            {
                name = "The Defias Brotherhood",
                questId = 166,
                level = 22,
                side = "Alliance",
                prereq = true,
                startNpc = "Gryan Stoutmantle", startNpcId = 234, startLoc = "Westfall", startCoords = "56.2 47.6",
                rewards = {
                    { itemId = 6087, name = "Chausses of Westfall" },
                    { itemId = 2041, name = "Tunic of Westfall" },
                    { itemId = 2042, name = "Staff of Westfall" },
                },
            },
            {
                name = "Underground Assault",
                questId = 2040,
                level = 20,
                side = "Alliance",
                prereq = false,
                startNpc = "Shoni the Silent", startNpcId = 6579, startLoc = "Stormwind City", startCoords = "55.6 12.6",
                rewards = {
                    { itemId = 7606, name = "Polar Gauntlets" },
                    { itemId = 7607, name = "Sable Wand" },
                },
            },
        },
    },
    {
        id = "shadowfang_keep",
        name = "Shadowfang Keep",
        zone = "Silverpine Forest",
        endBoss = "Archmage Arugal",
        bossLevel = 26,
        levelMin = 22,
        levelMax = 30,
        warnings = {
            "Low-level players can accidentally body pull when climbing the wooden stairs in the middle of the instance.",
            "[[Arugal]] casts Void Bolt which hits very hard, but can be avoided by using LoS at the base of the stairs.",
        },
        quests = {
            {
                name = "Arugal Must Die",
                questId = 1014,
                level = 27,
                side = "Horde",
                prereq = false,
                startNpc = "Dalar Dawnweaver", startNpcId = 1938, startLoc = "Silverpine Forest", startCoords = "44.2, 39.8",
                rewards = {
                    { itemId = 6414, name = "Seal of Sylvanas" },
                },
            },
            {
                name = "Deathstalkers in Shadowfang",
                questId = 1098,
                level = 25,
                side = "Horde",
                prereq = false,
                startNpc = "High Executor Hadrec", startNpcId = 1952, startLoc = "Silverpine Forest", startCoords = "43.4, 40.9",
                money = 1800,
                rewards = {
                    { itemId = 3324, name = "Ghostly Mantle" },
                },
            },
            {
                name = "The Book of Ur",
                questId = 1013,
                level = 26,
                side = "Horde",
                prereq = false,
                startNpc = "Keeper Bel'dugur", startNpcId = 2934, startLoc = "Undercity", startCoords = "53.7, 54.5",
                rewards = {
                    { itemId = 6335, name = "Grizzled Boots" },
                    { itemId = 4534, name = "Steel-clasped Bracers" },
                },
            },
        },
    },
    {
        id = "the_stockade",
        name = "The Stockade",
        zone = "Stormwind City",
        endBoss = "Bazil Thredd",
        bossLevel = 29,
        levelMin = 22,
        levelMax = 30,
        warnings = {
            "Defias Prisoners will repeatedly disarm the tank.",
            "[[Dextren Ward]] casts Intimidating Shout, which can fear players into nearby mobs."
        },
        quests = {
            {
                name = "Crime and Punishment",
                questId = 377,
                level = 26,
                side = "Alliance",
                prereq = false,
                startNpc = "Councilman Millstipe", startNpcId = 270, startLoc = "Duskwood", startCoords = "71.9, 47.8",
                rewards = {
                    { itemId = 2033, name = "Ambassador's Boots" },
                    { itemId = 2906, name = "Darkshire Mail Leggings" },
                },
            },
            {
                name = "Quell the Uprising",
                questId = 387,
                level = 26,
                side = "Alliance",
                prereq = false,
                startNpc = "Warden Thelwater", startNpcId = 1719, startLoc = "Stormwind City", startCoords = "42.0, 58.0",
                money = 4000,
            },
            {
                name = "The Color of Blood",
                questId = 388,
                level = 26,
                side = "Alliance",
                prereq = false,
                -- Nikova Raskol patrols the main road in Old Town
                startNpc = "Nikova Raskol", startNpcId = 1721, startLoc = "Stormwind City, wanders Old Town", startCoords = "73.0, 43.0",
                money = 4000,
            },
            {
                name = "The Fury Runs Deep",
                questId = 378,
                level = 27,
                side = "Alliance",
                prereq = true,
                startNpc = "Motley Garmason", startNpcId = 1074, startLoc = "Wetlands", startCoords = "49.7, 18.3",
                rewards = {
                    { itemId = 3562, name = "Belt of Vindication" },
                    { itemId = 1264, name = "Headbasher" },
                },
            },
            {
                name = "The Stockade Riots",
                questId = 391,
                level = 29,
                side = "Alliance",
                prereq = true,
                startNpc = "Warden Thelwater", startNpcId = 1719, startLoc = "Stormwind City", startCoords = "42.0, 58.0",
                money = 2500,
            },
            {
                name = "What Comes Around...",
                questId = 386,
                level = 25,
                side = "Alliance",
                prereq = false,
                startNpc = "Guard Berton", startNpcId = 859, startLoc = "Redridge Mountains", startCoords = "26.0, 47.0",
                rewards = {
                    { itemId = 1317, name = "Hardened Root Staff" },
                    { itemId = 3400, name = "Lucine Longsword" },
                },
            },
        },
    },
    {
        id = "blackfathom_deeps",
        name = "Blackfathom Deeps",
        zone = "Ashenvale",
        endBoss = "Aku'mai",
        bossLevel = 28,
        levelMin = 20,
        levelMax = 30,
        warnings = {
            "The ranged murlocs before [[Gelihast]] should be LoS pulled carefully.",
            "The candles in the room with [[Twilight Lord Kelris]] should be lit one at a time by a single player",
        },
        quests = {
            {
                name = "Allegiance to the Old Gods",
                questId = 6565,
                level = 26,
                side = "Horde",
                prereq = true,
                startNpc = "Je'neu Sancrea", startNpcId = 12736, startLoc = "Ashenvale", startCoords = "11.6, 34.2",
                money = 4000,
                rewards = {
                    { itemId = 17694, name = "Band of the Fist" },
                    { itemId = 17695, name = "Chestnut Mantle" },
                },
            },
            {
                name = "Amongst the Ruins",
                questId = 6921,
                level = 27,
                side = "Horde",
                prereq = false,
                startNpc = "Je'neu Sancrea", startNpcId = 12736, startLoc = "Ashenvale", startCoords = "11.6, 34.2",
                money = 4500,
            },
            {
                name = "Baron Aquanis",
                questId = 6922,
                level = 30,
                side = "Horde",
                prereq = false,
                startNpc = "Strange Water Globe",
                startItemId = 16782,
                startLoc = "Drops from Baron Aquanis",
                money = 0,
                rewards = {
                    { itemId = 16886, name = "Outlaw Sabre" },
                    { itemId = 16887, name = "Witch's Finger" },
                },
            },
            {
                name = "Blackfathom Villainy",
                questId = 1200,
                level = 27,
                side = "Alliance",
                prereq = false,
                startNpc = "Argent Guard Thaelrid", startNpcId = 4787, startLoc = "Inside the dungeon, in the southwestern cavern of the Ghamoo-ra area",
                money = 6500,
                rewards = {
                    { itemId = 7002, name = "Arctic Buckler" },
                    { itemId = 7001, name = "Gravestone Scepter" },
                },
            },
            {
                name = "Blackfathom Villainy",
                questId = 6561,
                level = 27,
                side = "Horde",
                prereq = false,
                startNpc = "Argent Guard Thaelrid", startNpcId = 4787, startLoc = "Inside the dungeon, in the southwestern cavern of the Ghamoo-ra area",
                money = 6500,
                rewards = {
                    { itemId = 7002, name = "Arctic Buckler" },
                    { itemId = 7001, name = "Gravestone Scepter" },
                },
            },
            {
                name = "Knowledge in the Deeps",
                questId = 971,
                level = 23,
                side = "Alliance",
                prereq = false,
                startNpc = "Gerrig Bonegrip", startNpcId = 2786, startLoc = "Ironforge", startCoords = "50.4, 5.6",
                money = 0,
                rewards = {
                    { itemId = 6743, name = "Sustaining Ring" },
                },
            },
            {
                name = "Researching the Corruption",
                questId = 1275,
                level = 24,
                side = "Alliance",
                prereq = true,
                startNpc = "Gershala Nightwhisper", startNpcId = 8997, startLoc = "Darkshore", startCoords = "44.1, 17.8",
                money = 3500,
                rewards = {
                    { itemId = 7003, name = "Beetle Clasps" },
                    { itemId = 7004, name = "Prelacy Cape" },
                },
            },
            {
                name = "The Essence of Aku'Mai",
                questId = 6563,
                level = 22,
                side = "Horde",
                prereq = false,
                startNpc = "Je'neu Sancrea", startNpcId = 12736, startLoc = "Ashenvale", startCoords = "11.6, 34.2",
                money = 1400,
            },
            {
                name = "Twilight Falls",
                questId = 1199,
                level = 25,
                side = "Alliance",
                prereq = false,
                startNpc = "Argent Guard Manados", startNpcId = 4784, startLoc = "Darnassus", startCoords = "55.4, 24.2",
                money = 0,
                rewards = {
                    { itemId = 7000, name = "Heartwood Girdle" },
                    { itemId = 6998, name = "Nimbus Boots" },
                },
            },
        },
    },
    {
        id = "gnomeregan",
        name = "Gnomeregan",
        zone = "Dun Morogh",
        endBoss = "Mekgineer Thermaplugg",
        bossLevel = 34,
        levelMin = 24,
        levelMax = 34,
        warnings = {  -- TODO: Verify these (I never do Gnomer)
            "After killing [[Grubbis]], the cave over his corpse will collapse, killing players underneath it.",
            "Low-level players or pets can body pull huge numbers of mobs in the two-tiered tunnel leading up to [[Mekgineer Thermaplugg]].",
        },
        quests = {
            {
                name = "A Fine Mess",
                questId = 2904,
                level = 30,
                side = "Both",
                prereq = false,
                startNpc = "Kernobee", startNpcId = 7850, startLoc = "Inside the dungeon, in the Dormitory",
                money = 0,
                rewards = {
                    { itemId = 9536, name = "Fairywing Mantle" },
                    { itemId = 9535, name = "Fire-welded Bracers" },
                },
            },
            {
                name = "Data Rescue",
                questId = 2930,
                level = 30,
                side = "Alliance",
                prereq = false,
                startNpc = "Master Mechanic Castpipe", startNpcId = 7950, startLoc = "Ironforge", startCoords = "69.4, 49.0",
                money = 0,
                rewards = {
                    { itemId = 9604, name = "Mechanic's Pipehammer" },
                    { itemId = 9605, name = "Repairman's Cape" },
                },
            },
            {
                name = "Essential Artificials",
                questId = 2924,
                level = 30,
                side = "Alliance",
                prereq = false,
                startNpc = "Klockmort Spannerspan", startNpcId = 6169, startLoc = "Ironforge", startCoords = "67.0, 46.0",
                money = 0,
            },
            {
                name = "Grime-Encrusted Ring",
                questId = 2945,
                level = 34,
                side = "Both",
                prereq = false,
                startNpc = "Grime-Encrusted Ring",
                startItemId = 9326,
                startLoc = "Drops from Dark Iron Agents inside the dungeon",
                money = 0,
            },
            {
                name = "Gyrodrillmatic Excavationators",
                questId = 2928,
                level = 30,
                side = "Alliance",
                prereq = false,
                startNpc = "Shoni the Silent", startNpcId = 6579, startLoc = "Stormwind City", startCoords = "55.6, 12.6",
                money = 0,
                rewards = {
                    { itemId = 9609, name = "Shilly Mitts" },
                    { itemId = 9608, name = "Shoni's Disarming Tool" },
                },
            },
            {
                name = "Rig Wars",
                questId = 2841,
                level = 35,
                side = "Horde",
                prereq = false,
                startNpc = "Nogg", startNpcId = 3412, startLoc = "Orgrimmar", startCoords = "75.8, 25.2",
                money = 0,
                rewards = {
                    { itemId = 9623, name = "Civinad Robes" },
                    { itemId = 9625, name = "Dual Reinforced Leggings" },
                    { itemId = 9624, name = "Triprunner Dungarees" },
                },
            },
            {
                name = "Save Techbot's Brain!",
                questId = 2922,
                level = 26,
                side = "Alliance",
                prereq = false,
                startNpc = "Tinkmaster Overspark", startNpcId = 7944, startLoc = "Ironforge", startCoords = "69.8, 50.0",
                money = 0,
            },
            {
                name = "The Grand Betrayal",
                questId = 2929,
                level = 35,
                side = "Alliance",
                prereq = false,
                startNpc = "High Tinker Mekkatorque", startNpcId = 7937, startLoc = "Ironforge", startCoords = "69.0, 49.0",
                money = 0,
                rewards = {
                    { itemId = 9623, name = "Civinad Robes" },
                    { itemId = 9625, name = "Dual Reinforced Leggings" },
                    { itemId = 9624, name = "Triprunner Dungarees" },
                },
            },
            {
                name = "The Only Cure is More Green Glow",
                questId = 2962,
                level = 26,
                side = "Alliance",
                prereq = true,
                startNpc = "Ozzie Togglevolt", startNpcId = 1268, startLoc = "Dun Morogh, Kharanos", startCoords = "45.0, 49.0",
                money = 0,
            },
        },
    },
    {
        id = "razorfen_kraul",
        name = "Razorfen Kraul",
        zone = "The Barrens",
        endBoss = "Charlga Razorflank",
        bossLevel = 33,
        levelMin = 30,
        levelMax = 40,
        warnings = {
            "[[Death Speaker Jargba]] will cast Mind Control on a random player.",
            "Bats will cast an AoE Silence - casters should stay outside melee range.",
        },
        quests = {
            {
                name = "A Vengeful Fate",
                questId = 1102,
                level = 29,
                side = "Horde",
                prereq = false,
                startNpc = "Auld Stonespire", startNpcId = 4451, startLoc = "Thunder Bluff", startCoords = "36.0, 60.0",
                money = 0,
                rewards = {
                    { itemId = 4197, name = "Berylline Pads" },
                    { itemId = 6725, name = "Marbled Buckler" },
                    { itemId = 6742, name = "Stonefist Girdle" },
                },
            },
            {
                name = "An Unholy Alliance",
                questId = 6522,
                level = 28,
                side = "Horde",
                prereq = false,
                startNpc = "Small Scroll",
                startItemId = 17008,
                startLoc = "Drops from Charlga Razorflank",
                money = 0,
            },
            {
                name = "Blueleaf Tubers",
                questId = 1221,
                level = 26,
                side = "Both",
                prereq = false,
                startNpc = "Mebok Mizzyrix", startNpcId = 3446, startLoc = "Ratchet", startCoords = "62.4, 37.6",
                money = 0,
                rewards = {
                    { itemId = 6755, name = "A Small Container of Gems" },
                },
            },
            {
                name = "Going, Going, Guano!",
                questId = 1109,
                level = 30,
                side = "Horde",
                prereq = false,
                startNpc = "Master Apothecary Faranell", startNpcId = 2055, startLoc = "Undercity", startCoords = "48.0, 69.0",
                money = 0,
            },
            {
                name = "Mortality Wanes",
                questId = 1142,
                level = 25,
                side = "Alliance",
                prereq = false,
                startNpc = "Heralath Fallowbrook", startNpcId = 4510, startLoc = "Inside the dungeon after the final boss",
                money = 0,
                rewards = {
                    { itemId = 6752, name = "Lancer Boots" },
                    { itemId = 6751, name = "Mourning Shawl" },
                },
            },
            {
                name = "The Crone of the Kraul",
                questId = 1101,
                level = 29,
                side = "Alliance",
                prereq = true,
                startNpc = "Falfindel Waywarder", startNpcId = 4048, startLoc = "Feralas", startCoords = "89.6, 46.6",
                money = 0,
                rewards = {
                    { itemId = 4197, name = "Berylline Pads" },
                    { itemId = 6725, name = "Marbled Buckler" },
                    { itemId = 6742, name = "Stonefist Girdle" },
                },
            },
            {
                name = "Willix the Importer",
                questId = 1144,
                level = 30,
                side = "Both",
                prereq = false,
                startNpc = "Willix the Importer", startNpcId = 4508, startLoc = "Inside the dungeon, behind the hut near Charlga Razorflank",
                money = 0,
                rewards = {
                    { itemId = 6748, name = "Monkey Ring" },
                    { itemId = 6750, name = "Snake Hoop" },
                    { itemId = 6749, name = "Tiger Band" },
                },
            },
        },
    },
    {
        id = "sm_graveyard",
        name = "Scarlet Monastery: Graveyard",
        zone = "Tirisfal Glades",
        endBoss = "Bloodmage Thalnos",
        bossLevel = 34,
        levelMin = 26,
        levelMax = 36,
        warnings = {
            "Pull carefully inside the crypt before the final boss to avoid multiple packs of enemies.",
            "Keep an eye out for three rare spawns: [[Azshir the Sleepless]], [[Fallen Champion]], and [[Ironspine]]."
        },
        quests = {
            {
                name = "Hearts of Zeal",
                questId = 1113,
                level = 33,
                side = "Horde",
                prereq = true,
                startNpc = "Master Apothecary Faranell", startNpcId = 2055, startLoc = "Undercity", startCoords = "48.0, 69.0",
                money = 0,
            },
            {
                name = "Vorrel's Revenge",
                questId = 1051,
                level = 33,
                side = "Horde",
                prereq = false,
                startNpc = "Vorrel Sengutz", startNpcId = 3981, startLoc = "Inside the dungeon, in the torture room after killing Interrogator Vishas",
                money = 0,
                rewards = {
                    { itemId = 4643, name = "Grimsteel Cape" },
                    { itemId = 7750, name = "Mantle of Woe" },
                    { itemId = 7751, name = "Vorrel's Boots" },
                },
            },
        },
    },
    {
        id = "sm_library",
        name = "Scarlet Monastery: Library",
        zone = "Tirisfal Glades",
        endBoss = "Arcanist Doan",
        bossLevel = 37,
        levelMin = 29,
        levelMax = 39,
        warnings = {
            "Scarlet Chaplains can Heal themselves, and should be interrupted.",
            "Scarlet Diviners cast a damaging Fireball, and should be interrupted.",
            "[[Arcanist Doan]] casts Detonation, a large AoE spell - run outside the door to LoS.",
        },
        quests = {
            {
                name = "Compendium of the Fallen",
                questId = 1049,
                level = 38,
                side = "Horde",
                prereq = false,
                startNpc = "Sage Truthseeker", startNpcId = 3978, startLoc = "Thunder Bluff", startCoords = "34.8, 47.8",
                money = 0,
                rewards = {
                    { itemId = 17508, name = "Forcestone Buckler" },
                    { itemId = 7749, name = "Omega Orb" },
                    { itemId = 7747, name = "Vile Protector" },
                },
            },
            {
                name = "In the Name of the Light",
                questId = 1053,
                level = 40,
                side = "Alliance",
                prereq = true,
                startNpc = "Raleigh the Devout", startNpcId = 3980, startLoc = "Hillsbrad Foothills", startCoords = "51.4, 58.6",
                money = 0,
                rewards = {
                    { itemId = 6831, name = "Black Menace" },
                    { itemId = 6830, name = "Bonebiter" },
                    { itemId = 11262, name = "Orb of Lorica" },
                    { itemId = 6829, name = "Sword of Serenity" },
                },
            },
            {
                name = "Into The Scarlet Monastery",
                questId = 1048,
                level = 42,
                side = "Horde",
                prereq = false,
                startNpc = "Varimathras", startNpcId = 2425, startLoc = "Undercity", startCoords = "56.2, 92.4",
                money = 0,
                rewards = {
                    { itemId = 10711, name = "Dragon's Blood Necklace" },
                    { itemId = 6803, name = "Prophetic Cane" },
                    { itemId = 6802, name = "Sword of Omen" },
                },
            },
            {
                name = "Mythology of the Titans",
                questId = 1050,
                level = 38,
                side = "Alliance",
                prereq = false,
                startNpc = "Librarian Mae Paledust", startNpcId = 3979, startLoc = "Ironforge", startCoords = "74.6, 12.6",
                money = 0,
                rewards = {
                    { itemId = 7746, name = "Explorers' League Commendation" },
                },
            },
            {
                name = "Test of Lore",
                questId = 1160,
                level = 36,
                side = "Horde",
                prereq = true,
                startNpc = "Parqual Fintallas", startNpcId = 4488, startLoc = "Undercity", startCoords = "57.8, 65",
                money = 0,
            },
        },
    },
    {
        id = "sm_armory",
        name = "Scarlet Monastery: Armory",
        zone = "Tirisfal Glades",
        endBoss = "Herod",
        bossLevel = 40,
        levelMin = 32,
        levelMax = 42,
        warnings = {
            "Scarlet Conjurors and Evokers cast a damaging Fireball, and should be interrupted.",
            "Scarlet Protectors cast Heal, and should be interrupted.",
            "Scarlet Myrmidons hit hard and enrage at low health.",
            "Scarlet Guardsman will disarm the tank.",
            "Any totems or dummies left in the hallway when [[Herod]] is engaged will aggro multiple elites.",
        },
        quests = {
            {
                name = "In the Name of the Light",
                questId = 1053,
                level = 40,
                side = "Alliance",
                prereq = true,
                startNpc = "Raleigh the Devout", startNpcId = 3980, startLoc = "Hillsbrad Foothills", startCoords = "51.4, 58.6",
                money = 0,
                rewards = {
                    { itemId = 6831, name = "Black Menace" },
                    { itemId = 6830, name = "Bonebiter" },
                    { itemId = 11262, name = "Orb of Lorica" },
                    { itemId = 6829, name = "Sword of Serenity" },
                },
            },
            {
                name = "Into The Scarlet Monastery",
                questId = 1048,
                level = 42,
                side = "Horde",
                prereq = false,
                startNpc = "Varimathras", startNpcId = 2425, startLoc = "Undercity", startCoords = "56.2, 92.4",
                money = 0,
                rewards = {
                    { itemId = 10711, name = "Dragon's Blood Necklace" },
                    { itemId = 6803, name = "Prophetic Cane" },
                    { itemId = 6802, name = "Sword of Omen" },
                },
            },
        },
    },
    {
        id = "sm_cathedral",
        name = "Scarlet Monastery: Cathedral",
        zone = "Tirisfal Glades",
        endBoss = "High Inquisitor Whitemane",
        bossLevel = 42,
        levelMin = 35,
        levelMax = 45,
        warnings = {
            "Scaret Abbot can cast Heal and Renew, and should be interrupted and prioritized as a kill target.",
            "All enemies inside the cathedral must be cleared, or they will aggro when [[Whitemane]] is engaged.",
        },
        quests = {
            {
                name = "In the Name of the Light",
                questId = 1053,
                level = 40,
                side = "Alliance",
                prereq = true,
                startNpc = "Raleigh the Devout", startNpcId = 3980, startLoc = "Hillsbrad Foothills", startCoords = "51.4, 58.6",
                money = 0,
                rewards = {
                    { itemId = 6831, name = "Black Menace" },
                    { itemId = 6830, name = "Bonebiter" },
                    { itemId = 11262, name = "Orb of Lorica" },
                    { itemId = 6829, name = "Sword of Serenity" },
                },
            },
            {
                name = "Into The Scarlet Monastery",
                questId = 1048,
                level = 42,
                side = "Horde",
                prereq = false,
                startNpc = "Varimathras", startNpcId = 2425, startLoc = "Undercity", startCoords = "56.2, 92.4",
                money = 0,
                rewards = {
                    { itemId = 10711, name = "Dragon's Blood Necklace" },
                    { itemId = 6803, name = "Prophetic Cane" },
                    { itemId = 6802, name = "Sword of Omen" },
                },
            },
        },
    },
    {
        id = "razorfen_downs",
        name = "Razorfen Downs",
        zone = "The Barrens",
        endBoss = "Amnennar the Coldbringer",
        bossLevel = 41,
        levelMin = 35,
        levelMax = 45,
        warnings = {
            "Roaches can social aggro nearby packs of enemies",
            "[[Amnennar]] is immune to Frost damage.",
            "[[Amnennar]] summons adds, which will need to be killed, CC'ed, or handled by burning the boss quickly.",
        },
        quests = {
            {
                name = "A Host of Evil",
                questId = 6626,
                level = 35,
                side = "Both",
                prereq = false,
                startNpc = "Myriam Moonsinger", startNpcId = 12866, startLoc = "The Barrens", startCoords = "49.0, 94.0",
                money = 7500,
            },
            {
                name = "An Unholy Alliance",
                questId = 6521,
                level = 36,
                side = "Horde",
                prereq = true,
                startNpc = "Varimathras", startNpcId = 2425, startLoc = "Undercity", startCoords = "56.2, 92.4",
                money = 2000,
                rewards = {
                    { itemId = 17042, name = "Nail Spitter" },
                    { itemId = 17039, name = "Skullbreaker" },
                    { itemId = 17043, name = "Zealot's Robe" },
                },
            },
            {
                name = "Bring the End",
                questId = 3341,
                level = 37,
                side = "Horde",
                prereq = false,
                startNpc = "Andrew Brownell", startNpcId = 2308, startLoc = "Undercity, Magic Quarter",
                money = 0,
                rewards = {
                    { itemId = 10824, name = "Amberglow Talisman" },
                    { itemId = 10823, name = "Vanquisher's Sword" },
                },
            },
            {
                name = "Bring the Light",
                questId = 3636,
                level = 39,
                side = "Alliance",
                prereq = false,
                startNpc = "Archbishop Benedictus", startNpcId = 1284, startLoc = "Stormwind City", startCoords = "39.6, 27.2",
                money = 0,
                rewards = {
                    { itemId = 10824, name = "Amberglow Talisman" },
                    { itemId = 10823, name = "Vanquisher's Sword" },
                },
            },
            {
                name = "Extinguishing the Idol",
                questId = 3525,
                level = 37,
                side = "Both",
                prereq = true,
                startNpc = "Belnistrasz", startNpcId = 8516, startLoc = "Inside the dungeon, before the spiral",
                money = 0,
                rewards = {
                    { itemId = 10710, name = "Dragonclaw Ring" },
                },
            },
            {
                name = "Scourge of the Downs",
                questId = 3523,
                level = 37,
                side = "Both",
                prereq = false,
                startNpc = "Belnistrasz", startNpcId = 8516, startLoc = "Inside the dungeon, before the spiral",
                money = 0,
            },
        },
    },
    {
        id = "uldaman",
        name = "Uldaman",
        zone = "Badlands",
        endBoss = "Archaedas",
        bossLevel = 47,
        levelMin = 42,
        levelMax = 52,
        warnings = {
            "The Trogg rooms before [[Grimlok]] are densely packed and enemies will flee at low health.",
            "Shadowforge dwarves near [[Galgann Firehammer]] can heal and cast from range; use LoS to pull them safely.",
            "[[Archaedas]] summons adds throughout the fight; burn him quickly or have a plan to control the adds.",
        },
        quests = {
            {
                name = "A Sign of Hope",
                questId = 721,
                level = 35,
                side = "Alliance",
                prereq = true,
                startNpc = "Prospector Ryedol", startNpcId = 2910, startLoc = "Badlands", startCoords = "53.0, 43.4",
                money = 1600,
            },
            {
                name = "Find the Gems",
                questId = 2201,
                level = 43,
                side = "Alliance",
                prereq = true,
                startNpc = "Remains of a Paladin", startNpcId = 6912, startLoc = "In the tunnel outside the dungeon portal",
                money = 2100,
            },
            {
                name = "Find the Gems and Power Source",
                questId = 2339,
                level = 44,
                side = "Horde",
                prereq = true,
                startNpc = "Jarkal Mossmeld", startNpcId = 6868, startLoc = "Badlands, Kargath", startCoords = "2.6, 46.0",
                money = 2200,
            },
            {
                name = "Necklace Recovery",
                questId = 2283,
                level = 41,
                side = "Horde",
                prereq = false,
                startNpc = "Dran Droffers", startNpcId = 6986, startLoc = "Orgrimmar", startCoords = "59.4, 36.4",
                money = 1400,
            },
            {
                name = "Reclaimed Treasures",
                questId = 1360,
                level = 43,
                side = "Alliance",
                prereq = false,
                startNpc = "Krom Stoutarm", startNpcId = 6294, startLoc = "Ironforge", startCoords = "74.6, 9.6",
                money = 6000,
            },
            {
                name = "Reclaimed Treasures",
                questId = 2342,
                level = 43,
                side = "Horde",
                prereq = false,
                startNpc = "Patrick Garrett", startNpcId = 5651, startLoc = "Undercity", startCoords = "62.0, 37.6",
                money = 6000,
            },
            {
                name = "Restoring the Necklace",
                questId = 2361,
                level = 44,
                side = "Alliance",
                prereq = true,
                startNpc = "Talvash del Kissel", startNpcId = 6826, startLoc = "Ironforge", startCoords = "36.2, 3.6",
                money = 3300,
                rewards = {
                    { itemId = 7673, name = "Talvash's Enhancing Necklace" },
                },
            },
            {
                name = "Solution to Doom",
                questId = 709,
                level = 40,
                side = "Both",
                prereq = false,
                startNpc = "Theldurin the Lost", startNpcId = 2785, startLoc = "Badlands", startCoords = "51.4, 76.8",
                money = 1800,
                rewards = {
                    { itemId = 4746, name = "Doomsayer's Robe" },
                },
            },
            {
                name = "The Hidden Chamber",
                questId = 2240,
                level = 40,
                side = "Alliance",
                prereq = true,
                startNpc = "Baelog", startNpcId = 6906, startLoc = "Inside the dungeon",
                money = 2300,
                rewards = {
                    { itemId = 9626, name = "Dwarven Charge" },
                    { itemId = 9627, name = "Explorer's League Lodestar" },
                },
            },
            {
                name = "The Lost Dwarves",
                questId = 2398,
                level = 40,
                side = "Alliance",
                prereq = false,
                startNpc = "Prospector Stormpike", startNpcId = 1356, startLoc = "Ironforge", startCoords = "74.4, 11.8",
                money = 500,
            },
            {
                name = "The Lost Tablets of Will",
                questId = 1139,
                level = 45,
                side = "Alliance",
                prereq = true,
                startNpc = "Advisor Belgrum", startNpcId = 2918, startLoc = "Ironforge", startCoords = "77.2, 9.4",
                money = 3000,
                rewards = {
                    { itemId = 6723, name = "Medal of Courage" },
                },
            },
            {
                name = "The Platinum Discs",
                questId = 2279,
                level = 47,
                side = "Alliance",
                prereq = false,
                startNpc = "The Discs of Norgannon", startLoc = "Interactive object in the room behind Archaedas",
                money = 3100,
            },
            {
                name = "The Platinum Discs",
                questId = 2280,
                level = 47,
                side = "Horde",
                prereq = false,
                startNpc = "The Discs of Norgannon", startLoc = "Interactive object in the room behind Archaedas",
                money = 3100,
            },
            {
                name = "The Shattered Necklace",
                questId = 2198,
                level = 41,
                side = "Alliance",
                prereq = false,
                startNpc = "Shattered Necklace", startItemId = 7666, startLoc = "Drops from enemies in and outside the dungeon",
                money = 1900,
            },
            {
                name = "Uldaman Reagent Run",
                questId = 17,
                level = 42,
                side = "Alliance",
                prereq = true,
                startNpc = "Ghak Healtouch", startNpcId = 1470, startLoc = "Loch Modan", startCoords = "37.0, 49.2",
                money = 5500,
                rewards = {
                    { itemId = 9030, name = "Restorative Potion" },
                },
            },
            {
                name = "Uldaman Reagent Run",
                questId = 2202,
                level = 42,
                side = "Horde",
                prereq = false,
                startNpc = "Jarkal Mossmeld", startNpcId = 6868, startLoc = "Badlands, Kargath", startCoords = "2.6, 46.0",
                money = 5500,
                rewards = {
                    { itemId = 9030, name = "Restorative Potion" },
                },
            },
        },
    },
    {
        id = "zul_farrak",
        name = "Zul'Farrak",
        zone = "Tanaris",
        endBoss = "Chief Ukorz Sandscalp",
        bossLevel = 48,
        levelMin = 44,
        levelMax = 54,
        warnings = {
            "Sandfury Shadowhunter casts Hex, which will polymorph a member of the group for 10 seconds.",
            "Pulling extra enemies during the stairs event can overwhelm the party.",
            "Packs of enemies surrounding [[Gahz'rilla]] and [[Hydromancer Velratha]] should be cleared before engaging them."
        },
        quests = {
            {
                name = "Divino-matic Rod",
                questId = 2768,
                level = 47,
                side = "Both",
                prereq = false,
                startNpc = "Chief Engineer Bilgewhizzle", startNpcId = 7407, startLoc = "Tanaris, Gadgetzan", startCoords = "52.6, 28.4",
                money = 0,
                rewards = {
                    { itemId = 9534, name = "Engineer's Guild Headpiece" },
                    { itemId = 9533, name = "Masons Fraternity Ring" },
                },
            },
            {
                name = "Gahz'rilla",
                questId = 2770,
                level = 50,
                side = "Both",
                prereq = false,
                startNpc = "Wizzle Brassbolts", startNpcId = 4453, startLoc = "Thousand Needles, Shimmering Flats", startCoords = "77.8, 77.0",
                money = 0,
                rewards = {
                    { itemId = 11122, name = "Carrot on a Stick" },
                },
            },
            {
                name = "Nekrum's Medallion",
                questId = 2991,
                level = 47,
                side = "Alliance",
                prereq = true,
                startNpc = "Thadius Grimshade", startNpcId = 8022, startLoc = "Blasted Lands", startCoords = "66.9, 19.5",
                money = 7000,
            },
            {
                name = "Scarab Shells",
                questId = 2865,
                level = 45,
                side = "Both",
                prereq = true,
                startNpc = "Tran'rek", startNpcId = 7876, startLoc = "Tanaris, Gadgetzan", startCoords = "51.6, 27.0",
                money = 6500,
            },
            {
                name = "The Prophecy of Mosh'aru",
                questId = 3527,
                level = 47,
                side = "Both",
                prereq = true,
                startNpc = "Yeh'kinya", startNpcId = 8579, startLoc = "Tanaris, Steamwheedle Port", startCoords = "67.0, 22.4",
                money = 0,
            },
            {
                name = "The Spider God",
                questId = 2936,
                level = 45,
                side = "Horde",
                prereq = true,
                startNpc = "Master Gadrin", startNpcId = 3188, startLoc = "Durotar, Sen'jin Village", startCoords = "56.0, 74.7",
                money = 0,
            },
            {
                name = "Tiara of the Deep",
                questId = 2846,
                level = 46,
                side = "Both",
                prereq = true,
                startNpc = "Tabetha", startNpcId = 6546, startLoc = "Dustwallow Marsh", startCoords = "46.0, 57.0",
                money = 6500,
                rewards = {
                    { itemId = 9531, name = "Gemshale Pauldrons" },
                    { itemId = 9527, name = "Spellshifter Rod" },
                },
            },
            {
                name = "Troll Temper",
                questId = 3042,
                level = 45,
                side = "Both",
                prereq = false,
                startNpc = "Trenton Lighthammer", startNpcId = 7804, startLoc = "Tanaris, Gadgetzan", startCoords = "51.4, 28.7",
                money = 19500,
            },
        },
    },
    {
        id = "maraudon",
        name = "Maraudon",
        zone = "Desolace",
        endBoss = "Princess Theradras",
        bossLevel = 51,
        levelMin = 46,
        levelMax = 55,
        warnings = {
            "Slime enemies leave behind a high damage poison cloud when they die - kite them as they are killed.",
            "[[Princess Theradras]] will periodically reset her threat and drops aggro.",
            "[[Princess Theradras]] casts Dust Field, which causes an AoE knockback.",
        },
        quests = {
            {
                name = "Corruption of Earth and Seed",
                questId = 7065,
                level = 51,
                side = "Alliance",
                prereq = false,
                startNpc = "Keeper Marandis", startNpcId = 13698, startLoc = "Desolace, Nijel's Point", startCoords = "63.0, 9.0",
                money = 3600,
                rewards = {
                    { itemId = 17743, name = "Resurgence Rod" },
                    { itemId = 17705, name = "Thrash Blade" },
                    { itemId = 17753, name = "Verdant Keeper's Aim" },
                },
            },
            {
                name = "Corruption of Earth and Seed",
                questId = 7064,
                level = 51,
                side = "Horde",
                prereq = false,
                startNpc = "Selendra", startNpcId = 13699, startLoc = "Desolace, Shadowprey Village", startCoords = "26.0, 76.0",
                money = 3600,
                rewards = {
                    { itemId = 17743, name = "Resurgence Rod" },
                    { itemId = 17705, name = "Thrash Blade" },
                    { itemId = 17753, name = "Verdant Keeper's Aim" },
                },
            },
            {
                name = "Legends of Maraudon",
                questId = 7044,
                level = 49,
                side = "Both",
                prereq = false,
                startNpc = "Cavindra", startNpcId = 13697, startLoc = "Inside the dungeon, in the orange path",
                money = 3600,
                rewards = {
                    { itemId = 17191, name = "Scepter of Celebras" },
                },
            },
            {
                name = "Seed of Life",
                questId = 7066,
                level = 51,
                side = "Both",
                prereq = true,
                startNpc = "Zaetar's Spirit", startNpcId = 12238, startLoc = "Inside the dungeon, after killing Princess Theradras",
                money = 5060,
            },
            {
                name = "Shadowshard Fragments",
                questId = 7070,
                level = 42,
                side = "Alliance",
                prereq = false,
                startNpc = "Archmage Tervosh", startNpcId = 4967, startLoc = "Dustwallow Marsh, Theramore Isle", startCoords = "66.2, 49.0",
                money = 2000,
                rewards = {
                    { itemId = 17773, name = "Prodigious Shadowshard Pendant" },
                    { itemId = 17772, name = "Zealous Shadowshard Pendant" },
                },
            },
            {
                name = "Shadowshard Fragments",
                questId = 7068,
                level = 42,
                side = "Horde",
                prereq = false,
                startNpc = "Uthel'nay", startNpcId = 7311, startLoc = "Orgrimmar", startCoords = "38.6, 86.4",
                money = 2000,
                rewards = {
                    { itemId = 17773, name = "Prodigious Shadowshard Pendant" },
                    { itemId = 17772, name = "Zealous Shadowshard Pendant" },
                },
            },
            {
                name = "The Pariah's Instructions",
                questId = 7067,
                level = 48,
                side = "Both",
                prereq = false,
                startNpc = "Centaur Pariah", startNpcId = 13717, startLoc = "Desolace", startCoords = "50.0, 86.0",
                money = 4070,
                rewards = {
                    { itemId = 17774, name = "Mark of the Chosen" },
                },
            },
            {
                name = "Twisted Evils",
                questId = 7028,
                level = 47,
                side = "Both",
                prereq = false,
                startNpc = "Willow", startNpcId = 13656, startLoc = "Desolace", startCoords = "62.2, 39.6",
                money = 3100,
                rewards = {
                    { itemId = 17775, name = "Acumen Robes" },
                    { itemId = 17779, name = "Hulkstone Pauldrons" },
                    { itemId = 17777, name = "Relentless Chain" },
                    { itemId = 17776, name = "Sprightring Helm" },
                },
            },
            {
                name = "Vyletongue Corruption",
                questId = 7041,
                level = 47,
                side = "Alliance",
                prereq = false,
                startNpc = "Talendria", startNpcId = 11715, startLoc = "Desolace, Nijel's Point", startCoords = "65.0, 9.0",
                money = 3100,
                rewards = {
                    { itemId = 17770, name = "Branchclaw Gauntlets" },
                    { itemId = 17778, name = "Sagebrush Girdle" },
                    { itemId = 17768, name = "Woodseed Hoop" },
                },
            },
            {
                name = "Vyletongue Corruption",
                questId = 7029,
                level = 47,
                side = "Horde",
                prereq = false,
                startNpc = "Vark Battlescar", startNpcId = 11823, startLoc = "Desolace, Shadowprey Village", startCoords = "24.0, 69.0",
                money = 3100,
                rewards = {
                    { itemId = 17770, name = "Branchclaw Gauntlets" },
                    { itemId = 17778, name = "Sagebrush Girdle" },
                    { itemId = 17768, name = "Woodseed Hoop" },
                },
            },
        },
    },
    {
        id = "sunken_temple",
        name = "Temple of Atal'Hakkar",
        zone = "Swamp of Sorrows",
        endBoss = "Shade of Eranikus",
        bossLevel = 55,
        levelMin = 50,
        levelMax = 60,
        warnings = {
            "Sunken Temple represents a significant step up in difficulty from previous dungeons!",
            "It is recommended to make your way through the outer dungeon to the instance as a group.",
            "[[Atal'alarion]] can aggro reset, and will knock players high into the air, causing them to take fall damage.",
            "The Atal'ai Deathwalkers prior to [[Jammal'an]] cast Fear, and turn into an unkillable ghost after dying which should be kited.",
            "During the [[Jammal'an the Prophet]] fight, kill [[Ogom the Wretched]] first.",
            "[[Jammal'an]] casts Hex to turn a party member into an Atal'ai Berserker - spread out and do not kill them.",
            "[[Weaver]], [[Dreamscythe]], [[Hazzas]], and [[Morphaz]] all have a frontal breath and knockback ability.",
            "[[Shade of Eranikus]] will summon any unkilled dragonkin when the fight begins.",
            "[[Shade of Eranikus]] will cast a 15 second sleep on the target with the highest threat.",
            "Set party loot rules to Free-for-all for the [[Avatar of Hakkar]] fight - adds will drop Hakkari Blood, which must be used to extinguish the braziers.",
            "[[Avatar of Hakker]] will cast a 6 second mind control on a random party member, causing them to enrage and attack a nearby ally.",
        },
        quests = {
            {
                name = "Haze of Evil",
                questId = 4143,
                level = 52,
                side = "Alliance",
                prereq = true,
                startNpc = "Gregan Brewspewer", startNpcId = 7775, startLoc = "Feralas", startCoords = "45.0, 25.0",
                money = 75,
            },
            {
                name = "Into The Temple of Atal'Hakkar",
                questId = 1475,
                level = 50,
                side = "Alliance",
                prereq = true,
                startNpc = "Brohann Caskbelly", startNpcId = 5384, startLoc = "Stormwind City, Dwarven District", startCoords = "64.2 20.8",
                money = 0,
                rewards = {
                    { itemId = 1490, name = "Guardian Talisman" },
                },
            },
            {
                name = "Into the Depths",
                questId = 3446,
                level = 51,
                side = "Both",
                prereq = true,
                startNpc = "Marvon Rivetseeker", startNpcId = 7771, startLoc = "Tanaris, Broken Pillar", startCoords = "52.6, 46.0",
                money = 0,
            },
            {
                name = "Jammal'an the Prophet",
                questId = 1446,
                level = 53,
                side = "Both",
                prereq = false,
                startNpc = "Atal'ai Exile", startNpcId = 5598, startLoc = "The Hinterlands", startCoords = "33.6, 75.2",
                money = 0,
                rewards = {
                    { itemId = 11124, name = "Helm of Exile" },
                    { itemId = 11123, name = "Rainstrider Leggings" },
                },
            },
            {
                name = "Secret of the Circle",
                questId = 3447,
                level = 51,
                side = "Both",
                prereq = true,
                startNpc = "Marvon Rivetseeker", startNpcId = 7771, startLoc = "Tanaris, Broken Pillar", startCoords = "52.6, 46.0",
                money = 0,
                rewards = {
                    { itemId = 10773, name = "Hakkari Urn" },
                },
            },
            {
                name = "The Essence of Eranikus",
                questId = 3373,
                level = 55,
                side = "Both",
                prereq = false,
                startNpc = "Essence of Eranikus",
                startItemId = 10454,
                startLoc = "Drops from Shade of Eranikus",
                money = 0,
                rewards = {
                    { itemId = 10455, name = "Chained Essence of Eranikus" },
                },
            },
            {
                name = "The God Hakkar",
                questId = 3528,
                level = 53,
                side = "Both",
                prereq = true,
                startNpc = "Yeh'kinya", startNpcId = 8579, startLoc = "Tanaris, Steamwheedle Port", startCoords = "67.0, 22.4",
                money = 24000,
                rewards = {
                    { itemId = 10749, name = "Avenguard Helm" },
                    { itemId = 10751, name = "Gemburst Circlet" },
                    { itemId = 10750, name = "Lifeforce Dirk" },
                },
            },
            {
                name = "The Temple of Atal'Hakkar",
                questId = 1445,
                level = 50,
                side = "Horde",
                prereq = true,
                startNpc = "Fel'zerul", startNpcId = 1443, startLoc = "Swamp of Sorrows, Stonard", startCoords = "48.0, 55.0",
                money = 0,
                rewards = {
                    { itemId = 1490, name = "Guardian Talisman" },
                },
            },
            {
                name = "Zapper Fuel",
                questId = 4146,
                level = 52,
                side = "Horde",
                prereq = true,
                startNpc = "Liv Rizzlefix", startNpcId = 8496, startLoc = "The Barrens, Ratchet", startCoords = "68.0, 73.0",
                money = 0,
            },
        },
    },
    {
        id = "blackrock_depths",
        name = "Blackrock Depths",
        zone = "Blackrock Mountain",
        endBoss = "Emperor Dagran Thaurissan",
        bossLevel = 56,
        levelMin = 48,
        levelMax = 60,
        warnings = {
            "BRD represents a significant step up in difficulty from previous dungeons!",
            "Later parts of BRD are more challenging than the earlier portions.",
            "[[Golem Lord Argelmach]] will pull any unkilled golems outside his room when engaged.",
            "The [[Grim Guzzler]] bar area can chain-pull and easily wipe groups.",
            "The [[Lyceum]] has fast-respawning non-elite dwarves. Stick together, use AoE, and move quickly.",
            "Any totems or dummies left in the hallway when [[Ambassador Flamelash]] is engaged will aggro multiple elites.",
            "[[Emperor Thaurissan]] hits very hard, and the fight is much more difficult when attempting to keep [[Princess Moira]] alive.",
        },
        quests = {
            {
                name = "A Taste of Flame",
                questId = 4024,
                level = 58,
                side = "Both",
                prereq = true,
                startNpc = "Cyrus Therepentous", startNpcId = 9459, startLoc = "Burning Steppes", startCoords = "95.0, 31.4",
                money = 26500,
                rewards = {
                    { itemId = 12066, name = "Shaleskin Cape" },
                    { itemId = 12083, name = "Valconian Sash" },
                    { itemId = 12082, name = "Wyrmhide Spaulders" },
                },
            },
            {
                name = "Attunement to the Core",
                questId = 7848,
                level = 60,
                side = "Both",
                prereq = false,
                startNpc = "Lothos Riftwaker", startNpcId = 14387, startLoc = "Blackrock Mountain, bottom of the chain",
                money = 39600,
            },
            {
                name = "Commander Gor'shak",
                questId = 3981,
                level = 52,
                side = "Horde",
                prereq = true,
                startNpc = "Galamav the Marksman", startNpcId = 9081, startLoc = "Badlands, Kargath", startCoords = "5.8, 47.6",
                money = 0,
            },
            {
                name = "Dark Iron Legacy",
                questId = 3802,
                level = 52,
                side = "Both",
                prereq = false,
                startNpc = "Franclorn Forgewright", startNpcId = 8888, startLoc = "Blackrock Mountain, inside the suspended room (Must be dead/Ghost)",
                money = 23000,
                rewards = {
                    { itemId = 11000, name = "Shadowforge Key" },
                },
            },
            {
                name = "Disharmony of Fire",
                questId = 3907,
                level = 56,
                side = "Horde",
                prereq = true,
                startNpc = "Thunderheart", startNpcId = 9084, startLoc = "Badlands, Kargath", startCoords = "3.6, 48.0",
                money = 25500,
                rewards = {
                    { itemId = 12112, name = "Crypt Demon Bracers" },
                    { itemId = 12114, name = "Nightfall Gloves" },
                    { itemId = 12115, name = "Stalwart Clutch" },
                    { itemId = 12113, name = "Sunborne Cape" },
                },
            },
            {
                name = "Hurley Blackbreath",
                questId = 4126,
                level = 55,
                side = "Alliance",
                prereq = true,
                startNpc = "Ragnar Thunderbrew", startNpcId = 1267, startLoc = "Dun Morogh, Kharanos", startCoords = "46.8, 52.4",
                money = 16500,
                rewards = {
                    { itemId = 12003, name = "Dark Dwarven Lager" },
                    { itemId = 12000, name = "Limb Cleaver" },
                    { itemId = 11964, name = "Swiftstrike Cudgel" },
                },
            },
            {
                name = "Incendius!",
                questId = 4263,
                level = 56,
                side = "Alliance",
                prereq = true,
                startNpc = "Jalinda Sprig", startNpcId = 9561, startLoc = "Burning Steppes, Morgan's Vigil", startCoords = "85.4, 70.0",
                money = 8500,
                rewards = {
                    { itemId = 12112, name = "Crypt Demon Bracers" },
                    { itemId = 12114, name = "Nightfall Gloves" },
                    { itemId = 12115, name = "Stalwart Clutch" },
                    { itemId = 12113, name = "Sunborne Cape" },
                },
            },
            {
                name = "Kharan Mighthammer",
                questId = 4341,
                level = 59,
                side = "Alliance",
                prereq = true,
                startNpc = "King Magni Bronzebeard", startNpcId = 2784, startLoc = "Ironforge", startCoords = "40.0, 55.2",
                money = 0,
            },
            {
                name = "KILL ON SIGHT: Dark Iron Dwarves",
                questId = 4081,
                level = 52,
                side = "Horde",
                prereq = false,
                startNpc = "Wanted Poster", startNpcId = 164867, startLoc = "Badlands, Kargath", startCoords = "3.9, 47.4",
                money = 15500,
            },
            {
                name = "KILL ON SIGHT: High Ranking Dark Iron Officials",
                questId = 4082,
                level = 54,
                side = "Horde",
                prereq = true,
                startNpc = "Wanted Poster", startNpcId = 164868, startLoc = "Badlands, Kargath", startCoords = "4.0, 46.8",
                money = 16500,
            },
            {
                name = "Lost Thunderbrew Recipe",
                questId = 4134,
                level = 55,
                side = "Horde",
                prereq = false,
                startNpc = "Shadowmage Vivian Lagrave", startNpcId = 9078, startLoc = "Badlands, Kargath", startCoords = "3.2, 47.8",
                money = 8500,
                rewards = {
                    { itemId = 6149, name = "Greater Mana Potion" },
                    { itemId = 12000, name = "Limb Cleaver" },
                    { itemId = 3928, name = "Superior Healing Potion" },
                    { itemId = 11964, name = "Swiftstrike Cudgel" },
                },
            },
            {
                name = "Marshal Windsor",
                questId = 4241,
                level = 48,
                side = "Alliance",
                prereq = true,
                startNpc = "Marshal Maxwell", startNpcId = 9560, startLoc = "Burning Steppes, Morgan's Vigil", startCoords = "84.8, 68.6",
                money = 0,
            },
            {
                name = "Operation: Death to Angerforge",
                questId = 4132,
                level = 58,
                side = "Horde",
                prereq = true,
                startNpc = "Warlord Goretooth", startNpcId = 9077, startLoc = "Badlands, Kargath", startCoords = "5.8, 47.6",
                money = 26500,
                rewards = {
                    { itemId = 12059, name = "Conqueror's Medallion" },
                },
            },
            {
                name = "Ribbly Screwspigot",
                questId = 4136,
                level = 53,
                side = "Both",
                prereq = true,
                startNpc = "Yuka Screwspigot", startNpcId = 9544, startLoc = "Burning Steppes", startCoords = "66.0, 22.0",
                money = 6000,
                rewards = {
                    { itemId = 11963, name = "Penance Spaulders" },
                    { itemId = 11865, name = "Rancor Boots" },
                    { itemId = 12049, name = "Splintsteel Armor" },
                },
            },
            {
                name = "The Fate of the Kingdom",
                questId = 4362,
                level = 59,
                side = "Alliance",
                prereq = true,
                startNpc = "King Magni Bronzebeard", startNpcId = 2784, startLoc = "Ironforge", startCoords = "40.0, 55.2",
                money = 0,
                rewards = {
                    { itemId = 12548, name = "Magni's Will" },
                    { itemId = 12543, name = "Songstone of Ironforge" },
                },
            },
            {
                name = "The Good Stuff",
                questId = 4286,
                level = 56,
                side = "Alliance",
                prereq = false,
                startNpc = "Oralius", startNpcId = 9177, startLoc = "Burning Steppes, Morgan's Vigil", startCoords = "84.6, 68.6",
                money = 8500,
                rewards = {
                    { itemId = 11883, name = "A Dingy Fanny Pack" },
                },
            },
            {
                name = "The Heart of the Mountain",
                questId = 4123,
                level = 55,
                side = "Both",
                prereq = false,
                startNpc = "Maxwort Uberglint", startNpcId = 9536, startLoc = "Burning Steppes", startCoords = "65.2, 23.8",
                money = 8500,
            },
            {
                name = "The Last Element",
                questId = 7201,
                level = 54,
                side = "Horde",
                prereq = true,
                startNpc = "Shadowmage Vivian Lagrave", startNpcId = 9078, startLoc = "Badlands, Kargath", startCoords = "3.2, 47.8",
                money = 24500,
                rewards = {
                    { itemId = 12038, name = "Lagrave's Seal" },
                },
            },
            {
                name = "The Love Potion",
                questId = 4201,
                level = 54,
                side = "Both",
                prereq = false,
                startNpc = "Mistress Nagmara", startNpcId = 9500, startLoc = "Inside the dungeon, in the Grim Guzzler",
                money = 8000,
                rewards = {
                    { itemId = 11962, name = "Manacle Cuffs" },
                    { itemId = 11866, name = "Nagmara's Whipping Belt" },
                },
            },
            {
                name = "The Rise of the Machines",
                questId = 4063,
                level = 58,
                side = "Horde",
                prereq = true,
                startNpc = "Lotwil Veriatus", startNpcId = 2921, startLoc = "Badlands", startCoords = "26.0, 45.6",
                money = 26500,
                rewards = {
                    { itemId = 12109, name = "Azure Moon Amice" },
                    { itemId = 12110, name = "Raincaster Drape" },
                    { itemId = 12108, name = "Basaltscale Armor" },
                    { itemId = 12111, name = "Lavaplate Gauntlets" },
                }
            },
            {
                name = "The Royal Rescue",
                questId = 4003,
                level = 59,
                side = "Horde",
                prereq = true,
                startNpc = "Thrall", startNpcId = 4949, startLoc = "Orgrimmar", startCoords = "32.0, 37.8",
                money = 0,
                rewards = {
                    { itemId = 12545, name = "Eye of Orgrimmar" },
                    { itemId = 12544, name = "Thrall's Resolve" },
                },
            },
        },
    },
    {
        id = "lower_blackrock_spire",
        name = "Lower Blackrock Spire",
        zone = "Blackrock Mountain",
        endBoss = "Overlord Wyrmthalak",
        bossLevel = 60,
        levelMin = 55,
        levelMax = 60,
        warnings = {
            "LBRS is densely packed with elite groups which should be pulled carefully.",
            "[[Highlord Omokk]] has two adds and enrages at low HP. CC the adds and tank him against a wall to avoid his cleave and knockback.",
            "[[Shadow Hunter Vosh'gajin]] casts Hex on multiple players and Curse of Blood - decurse immediately and CC the adds.",
            "[[War Master Voone]] hits extremely hard and casts Cleave.",
            "[[Mother Smolderweb]] casts a frontal stun and Mother's Milk (dangerous AoE poison stun).",
            "The [[Urok Doomhowl]] fight is a challenging gauntlet; and the boss will cast Fear.",
            "[[Gizrul the Slavener]] charges in immediately after [[Halycon]] dies; eat and drink quickly between the two fights.",
            "[[Overlord Wyrmthalak]] summons two elite adds at 50% HP. He casts Cleave and Sweeping Slam - stay behind and at range.",
        },
        quests = {
            {
                -- https://www.wowhead.com/classic/quest=5001/bijous-belongings
                name = "Bijou's Belongings",
                questId = 5001,
                level = 59,
                side = "Alliance",
                prereq = false,
                -- https://www.wowhead.com/classic/npc=10257/bijou
                startNpc = "Bijou", startNpcId = 10257, startLoc = "Lower Blackrock Spire, on a ledge overlooking Hordemar City",
            },
            {
                -- https://www.wowhead.com/classic/quest=4982/bijous-belongings
                name = "Bijou's Belongings",
                questId = 4982,
                level = 59,
                side = "Horde",
                prereq = false,
                -- https://www.wowhead.com/classic/npc=10257/bijou
                startNpc = "Bijou", startNpcId = 10257, startLoc = "Lower Blackrock Spire, on a ledge overlooking Hordemar City",
            },
            {
                name = "En-Ay-Es-Tee-Why",
                questId = 4862,
                level = 59,
                side = "Both",
                prereq = false,
                startNpc = "Kibler", startNpcId = 10260, startLoc = "Burning Steppes, Flame Crest", startCoords = "65.9, 21.9",
                money = 9000,
                rewards = {
                    { itemId = 12529, name = "Smolderweb Carrier" },
                },
            },
            {
                -- https://www.wowhead.com/classic/quest=5089/general-drakkisaths-command
                name = "General Drakkisath's Command",
                questId = 5089,
                level = 60,
                side = "Alliance",
                prereq = false,
                startNpc = "General Drakkisath's Command", startItemId = 12780, startLoc = "Drops from Overlord Wyrmthalak",
                money = 39600,
            },
            {
                name = "Kibler's Exotic Pets",
                questId = 4729,
                level = 59,
                side = "Both",
                prereq = false,
                startNpc = "Kibler", startNpcId = 10260, startLoc = "Burning Steppes, Flame Crest", startCoords = "65.9, 21.9",
                money = 9000,
                rewards = {
                    { itemId = 12264, name = "Worg Carrier" },
                },
            },
            {
                name = "Maxwell's Mission",
                questId = 5081,
                level = 60,
                side = "Alliance",
                prereq = true,
                startNpc = "Marshal Maxwell", startNpcId = 9560, startLoc = "Burning Steppes, Morgan's Vigil", startCoords = "84.7, 69.0",
                money = 18000,
                rewards = {
                    { itemId = 13961, name = "Halycon's Muzzle" },
                    { itemId = 13959, name = "Omokk's Girth Restrainer" },
                    { itemId = 13962, name = "Vosh'gajin's Strand" },
                    { itemId = 13963, name = "Voone's Vice Grips" },
                    { itemId = 13958, name = "Wyrmthalak's Shackles" },
                },
            },
            {
                -- https://www.wowhead.com/classic/quest=4866/mothers-milk
                name = "Mother's Milk",
                questId = 4866,
                level = 60,
                side = "Both",
                prereq = false,
                -- https://www.wowhead.com/classic/npc=9563/ragged-john
                startNpc = "Ragged John", startNpcId = 9563, startLoc = "Burning Steppes, Flame Crest", startCoords = "65.0, 23.8",
                money = 18000,
                rewards = {
                    { itemId = 15873, name = "Ragged John's Neverending Cup" },
                },
            },
            {
                name = "Operative Bijou",
                questId = 4981,
                level = 59,
                side = "Horde",
                prereq = false,
                startNpc = "Lexlort", startNpcId = 9080, startLoc = "Badlands, Kargath", startCoords = "5.9, 47.6",
            },
            {
                name = "Put Her Down",
                questId = 4701,
                level = 59,
                side = "Alliance",
                prereq = false,
                startNpc = "Helendis Riverhorn", startNpcId = 9562, startLoc = "Burning Steppes, Morgan's Vigil", startCoords = "85.8, 69.0",
                money = 18000,
                rewards = {
                    { itemId = 15824, name = "Astoria Robes" },
                    { itemId = 15827, name = "Jadescale Breastplate" },
                    { itemId = 15825, name = "Traphook Jerkin" },
                },
            },
            {
                -- https://www.wowhead.com/classic/quest=4742/seal-of-ascension
                name = "Seal of Ascension",
                questId = 4742,
                level = 60,
                side = "Both",
                prereq = false,
                -- Vaelan is inside the dungeon, disguised as a Scarshield Infiltrator; no world coordinates
                -- https://www.wowhead.com/classic/npc=10296/vaelan
                startNpc = "Vaelan", startNpcId = 10296, startLoc = "Lower Blackrock Spire, disguised as Scarshield Infiltrator on a ledge in Hordemar City",
                money = 49800,
            },
            {
                name = "The Final Tablets",
                questId = 4788,
                level = 58,
                side = "Both",
                prereq = true,
                startNpc = "Prospector Ironboot", startNpcId = 10460, startLoc = "Tanaris, Steamwheedle Port", startCoords = "66.9, 24.0",
            },
            {
                name = "The Pack Mistress",
                questId = 4724,
                level = 59,
                side = "Horde",
                prereq = false,
                startNpc = "Galamav the Marksman", startNpcId = 9081, startLoc = "Badlands, Kargath", startCoords = "6.0, 47.7",
                money = 18000,
                rewards = {
                    { itemId = 15824, name = "Astoria Robes" },
                    { itemId = 15827, name = "Jadescale Breastplate" },
                    { itemId = 15825, name = "Traphook Jerkin" },
                },
            },
            {
                -- https://www.wowhead.com/classic/quest=4867/urok-doomhowl
                name = "Urok Doomhowl",
                questId = 4867,
                level = 60,
                side = "Both",
                prereq = false,
                -- Warosh is inside the dungeon instance; no world coordinates
                -- https://www.wowhead.com/classic/npc=10799/warosh
                startNpc = "Warosh", startNpcId = 10799, startLoc = "Lower Blackrock Spire, wandering in Hordemar City",
                money = 59700,
                rewards = {
                    { itemId = 15867, name = "Prismcharm" },
                },
            },
            {
                name = "Warlord's Command",
                questId = 4903,
                level = 60,
                side = "Horde",
                prereq = false,
                startNpc = "Warlord Goretooth", startNpcId = 9077, startLoc = "Badlands, Kargath", startCoords = "5.8, 47.5",
                money = 18000,
                rewards = {
                    { itemId = 13961, name = "Halycon's Muzzle" },
                    { itemId = 13959, name = "Omokk's Girth Restrainer" },
                    { itemId = 13962, name = "Vosh'gajin's Strand" },
                    { itemId = 13963, name = "Voone's Vice Grips" },
                    { itemId = 13958, name = "Wyrmthalak's Shackles" },
                },
            },
        },
    },
    {
        id = "upper_blackrock_spire",
        name = "Upper Blackrock Spire",
        zone = "Blackrock Mountain",
        endBoss = "General Drakkisath",
        bossLevel = 62,
        levelMin = 55,
        levelMax = 60,
        warnings = {
            "UBRS mobs hit significantly harder than standard 5-man dungeons.",
            "The Rookery is full of dragon eggs that hatch whelps on contact.",
            "[[Warchief Rend Blackhand]] arrives mounted on [[Gyth]] after waves of dragonkin. Kill Gyth first. Avoid Rend's Whirlwind and Thunderclap.",
            "[[The Beast]] casts Fear and Flamebreak (knockback + fire AoE). Tank against a wall. Ranged stay at max range to outrange both.",
            "Clear all stealthed Blackhand Assassins near [[The Beast]]'s room before engaging — they will ambush during the fight.",
            "[[General Drakkisath]] has two Chromatic Elite Guard adds - CC or off-tank them. Kill both guards before engaging the General.",
            "[[General Drakkisath]] casts Conflagration on the tank, incapacitating them. An off-tank must taunt immediately.",
        },
        quests = {
            {
                -- https://www.wowhead.com/classic/quest=7761/blackhands-command
                name = "Blackhand's Command",
                questId = 7761,
                level = 60,
                side = "Both",
                prereq = false,
                startNpc = "Blackhand's Command", startItemId = 18987, startLoc = "Drops from Scarshield Quartermaster outside Blackrock Spire entrance",
                money = 39600,
            },
            {
                --https://www.wowhead.com/classic/quest=4764/doomriggers-clasp
                name = "Doomrigger's Clasp",
                questId = 4764,
                level = 60,
                side = "Alliance",
                prereq = false,
                -- https://www.wowhead.com/classic/npc=9565/mayara-brightwing
                startNpc = "Mayara Brightwing", startNpcId = 9565, startLoc = "Burning Steppes, Morgan's Vigil", startCoords = "84.8, 69.1",
                money = 9900,
            },
            {
                --https://www.wowhead.com/classic/quest=6821/eye-of-the-emberseer
                name = "Eye of the Emberseer",
                questId = 6821,
                level = 60,
                side = "Both",
                prereq = true,
                -- https://www.wowhead.com/classic/npc=13278/duke-hydraxis
                startNpc = "Duke Hydraxis", startNpcId = 13278, startLoc = "Azshara, Bay of Storms island", startCoords = "79.3, 73.7",
                money = 49800,
            },
            {
                -- https://www.wowhead.com/classic/quest=4974/for-the-horde
                name = "For The Horde!",
                questId = 4974,
                level = 60,
                side = "Horde",
                prereq = true,
                -- https://www.wowhead.com/classic/npc=4949/thrall
                startNpc = "Thrall", startNpcId = 4949, startLoc = "Orgrimmar, Grommash Hold", startCoords = "31.6, 37.8",
                money = 27000,
                rewards = {
                    { itemId = 13965, name = "Blackhand's Breadth" },
                    { itemId = 13968, name = "Eye of the Beast" },
                    { itemId = 13966, name = "Mark of Tyranny" },
                },
            },
            {
                -- https://www.wowhead.com/classic/quest=5102/general-drakkisaths-demise
                name = "General Drakkisath's Demise",
                questId = 5102,
                level = 60,
                side = "Alliance",
                prereq = true,
                -- https://www.wowhead.com/classic/npc=9560/marshal-maxwell
                startNpc = "Marshal Maxwell", startNpcId = 9560, startLoc = "Burning Steppes, Morgan's Vigil", startCoords = "84.7, 69.0",
                money = 27000,
                rewards = {
                    { itemId = 13965, name = "Blackhand's Breadth" },
                    { itemId = 13968, name = "Eye of the Beast" },
                    { itemId = 13966, name = "Mark of Tyranny" },
                },
            },
            {
                -- https://www.wowhead.com/classic/quest=4768/the-darkstone-tablet
                name = "The Darkstone Tablet",
                questId = 4768,
                level = 60,
                side = "Horde",
                prereq = false,
                -- https://www.wowhead.com/classic/npc=9078/shadowmage-vivian-lagrave
                startNpc = "Shadowmage Vivian Lagrave", startNpcId = 9078, startLoc = "Badlands, Kargath", startCoords = "2.9, 47.8",
                money = 27000,
                rewards = {
                    { itemId = 15860, name = "Blinkstrike Armguards" },
                    { itemId = 15861, name = "Swiftfoot Treads" },
                },
            },
            {
                -- https://www.wowhead.com/classic/quest=5160/the-matron-protectorate
                name = "The Matron Protectorate",
                questId = 5160,
                level = 60,
                side = "Both",
                prereq = false,
                -- Awbee is inside the dungeon instance; no world coordinates
                startNpc = "Awbee", startNpcId = 10740, startLoc = "Upper Blackrock Spire, blue dragon whelpling found after The Beast's room",
                money = 39600,
            },
        },
    },
    {
        id = "dire_maul_east",
        name = "Dire Maul: East",
        zone = "Feralas",
        endBoss = "Alzzin the Wildshaper",
        bossLevel = 58,
        levelMin = 54,
        levelMax = 60,
        warnings = {
            "[[Pusillin]] summons Wildspawn Imps and gains Spirit of Runn Tum (50% spell reflect).",
            "[[Lethtendris]] casts Void Bolt and Shadow Bolt Volley, which should be interrupted as much as possible.",
            "[[Hydrospawn]] summons Hydroling adds that hit hard.",
            "[[Zevrim Thornhoof]] casts Sacrifice, teleporting a player to his altar for heavy damage. Heal them immediately or they will die.",
            "Clear all Warpwood Crushers on the ramp before engaging [[Alzzin the Wildshaper]], as they will join the fight if left alive.",
            "[[Alzzin the Wildshaper]] drops all threat each time he shapeshifts. Let the tank re-establish before DPSing.",
            "[[Alzzin the Wildshaper]] summons imps at 35% HP. AoE them down quickly.",
        },
        quests = {
            {
                name = "Lethtendris's Web",
                questId = 7488,
                level = 57,
                side = "Alliance",
                prereq = false,
                startNpc = "Latronicus Moonspear", startNpcId = 7877, startLoc = "Feralas, Feathermoon Stronghold", startCoords = "30.4, 46.2",
                money = 17000,
                rewards = {
                    { itemId = 18491, name = "Lorespinner" },
                },
            },
            {
                name = "Lethtendris's Web",
                questId = 7489,
                level = 57,
                side = "Horde",
                prereq = false,
                startNpc = "Talo Thornhoof", startNpcId = 7776, startLoc = "Feralas, Camp Mojache", startCoords = "76.2, 43.8",
                money = 17000,
                rewards = {
                    { itemId = 18491, name = "Lorespinner" },
                },
            },
            {
                name = "Pusillin and the Elder Azj'Tordin",
                questId = 7441,
                level = 58,
                side = "Both",
                prereq = false,
                startNpc = "Azj'Tordin", startNpcId = 14355, startLoc = "Feralas, Lariss Pavilion", startCoords = "76.9, 37.4",
                money = 17500,
                rewards = {
                    { itemId = 18410, name = "Sprinter's Sword" },
                    { itemId = 18411, name = "Spry Boots" },
                },
            },
            {
                name = "Shards of the Felvine",
                questId = 5526,
                level = 60,
                side = "Both",
                prereq = true,
                startNpc = "Rabine Saturna", startNpcId = 11801, startLoc = "Moonglade, Nighthaven", startCoords = "51.7, 45.1",
                rewards = {
                    { itemId = 18536, name = "Milli's Lexicon" },
                    { itemId = 18535, name = "Milli's Shield" },
                },
            },
        },
    },
    {
        id = "dire_maul_west",
        name = "Dire Maul: West",
        zone = "Feralas",
        endBoss = "Prince Tortheldrin",
        bossLevel = 61,
        levelMin = 54,
        levelMax = 60,
        warnings = {
            "[[Tendris Warpwood]] will summon any remaining living Ironbark Protectors when engaged.",
            "[[Magister Kalendris]] casts Dominate Mind on a random player.",
            "[[Magister Kalendris]] casts Mind Blast and Mind Flay, which should be interrupted as much as possible.",
            "Stack in melee range against [[Illyanna Ravenoak]] to prevent her from using Volley and Multi-Shot.",
            "[[Immol'thar]] teleports random players to the center, dropping their threat. Stop DPS if the tank is teleported.",
            "[[Immol'thar]] enrages at 50% HP. Save cooldowns for the second half of the fight.",
            "[[Prince Tortheldrin]] casts Arcane Blast, which knocks back and resets all threat.",
        },
        quests = {
            {
                name = "Elven Legends",
                questId = 7482,
                level = 60,
                side = "Alliance",
                prereq = false,
                startNpc = "Scholar Runethorn", startNpcId = 14374, startLoc = "Feralas, Feathermoon Stronghold", startCoords = "31.2, 43.9",
                money = 18000,
            },
            {
                name = "Elven Legends",
                questId = 7481,
                level = 60,
                side = "Horde",
                prereq = false,
                startNpc = "Sage Korolusk", startNpcId = 14373, startLoc = "Feralas, Camp Mojache", startCoords = "75.2, 43.8",
                money = 18000,
            },
            {
                name = "The Madness Within",
                questId = 7461,
                level = 60,
                side = "Both",
                prereq = false,
                -- Shen'dralar Ancient is inside the dungeon instance; no world coordinates
                startNpc = "Shen'dralar Ancient", startNpcId = 14358, startLoc = "Dire Maul West, on the bridge in the Court of the Highborne",
                money = 59700,
            },
            {
                name = "The Treasure of the Shen'dralar",
                questId = 7877,
                level = 60,
                side = "Both",
                prereq = true,
                -- Shen'dralar Ancient is inside the dungeon instance; no world coordinates
                startNpc = "Shen'dralar Ancient", startNpcId = 14358, startLoc = "Dire Maul West, on the bridge in the Court of the Highborne",
                money = 27000,
                rewards = {
                    { itemId = 18421, name = "Backwood Helm" },
                    { itemId = 18420, name = "Bonecrusher" },
                    { itemId = 18424, name = "Sedge Boots" },
                },
            },
            {
                name = "Unfinished Gordok Business",
                questId = 7703,
                level = 60,
                side = "Both",
                prereq = true,
                -- Captain Kromcrush is inside Dire Maul North; objective is in Dire Maul West
                startNpc = "Captain Kromcrush", startNpcId = 14325, startLoc = "Dire Maul North, requires completed tribute run",
                rewards = {
                    { itemId = 18367, name = "Gordok's Gauntlets" },
                    { itemId = 18368, name = "Gordok's Gloves" },
                    { itemId = 18366, name = "Gordok's Handguards" },
                    { itemId = 18369, name = "Gordok's Handwraps" },
                },
            },
        },
    },
    {
        id = "dire_maul_north",
        name = "Dire Maul: North",
        zone = "Feralas",
        endBoss = "King Gordok",
        bossLevel = 61,
        levelMin = 54,
        levelMax = 60,
        warnings = {
            "Dire Maul: North is one of the most difficult level 60 dungeons!",
            "Gordok ogre packs are densely packed. Pull carefully to avoid chain pulls.",
            "All three Guards have Knock Away and Shield Charge - tank against a wall to avoid being punted.",
            "[[Stomper Kreeg]] casts Whirlwind - melee should back out when it starts.",
            "[[Captain Kromcrush]] casts Intimidating Shout, which can fear players into nearby packs. Clear the area first.",
            "[[Captain Kromcrush]] spawns Gordok Reaver adds at 50% HP. Be ready to CC or burn them.",
            "[[Cho'Rush the Observer]] casts Healing Wave on [[King Gordok]] - this must be interrupted.",
            "[[Cho'Rush the Observer]] casts Psychic Scream - use fear protection or stay at max range.",
            "[[King Gordok]] enrages at low HP. Save cooldowns for the end.",
        },
        quests = {
            {
                name = "Free Knot!",
                questId = 5525,
                level = 60,
                side = "Both",
                prereq = false,
                startNpc = "Knot Thimblejack", startNpcId = 14338, startLoc = "Dire Maul North, chained near Guard Slip'kik",
                money = 39600
            },
            {
                name = "The Gordok Ogre Suit",
                questId = 5518,
                level = 60,
                side = "Both",
                prereq = false,
                startNpc = "Knot Thimblejack", startNpcId = 14338, startLoc = "Dire Maul North, chained near Guard Slip'kik",
                rewards = {
                    { itemId = 18258, name = "Gordok Ogre Suit" },
                },
            },
            {
                name = "Unfinished Gordok Business",
                questId = 7703,
                level = 60,
                side = "Both",
                prereq = true,
                startNpc = "Captain Kromcrush", startNpcId = 14325, startLoc = "Dire Maul North, requires completed tribute run",
                money = 49800,
                rewards = {
                    { itemId = 18367, name = "Gordok's Gauntlets" },
                    { itemId = 18368, name = "Gordok's Gloves" },
                    { itemId = 18366, name = "Gordok's Handguards" },
                    { itemId = 18369, name = "Gordok's Handwraps" },
                },
            },
        },
    },
    {
        id = "stratholme_living",
        name = "Stratholme: Living",
        zone = "Eastern Plaguelands",
        endBoss = "Balnazzar",
        bossLevel = 62,
        levelMin = 58,
        levelMax = 60,
        warnings = {
            "Scarlet Crusade mobs in the Living wing call for help. Pull carefully to avoid chain-pulling entire rooms.",
            "[[Timmy the Cruel]] enrages at low HP - save defensive cooldowns for the burn phase.",
            "[[Cannon Master Willey]] continuously spawns Crimson Rifleman. Use the cannons in the room against them, or they will overwhelm the group.",
            "[[Archivist Galford]] casts Burning Winds, which can cause him to swap targets. Loot quest items from the library BEFORE burning the archive.",
            "[[Balnazzar]] (disguised as [[Grand Crusader Dathrohan]]) transforms into demon form at 40% HP. He casts Psychic Scream - ranged stay at max range to outrange it.",
            "[[Balnazzar]] casts Deep Sleep on random players - dispel it immediately.",
        },
        quests = {
            {
                -- https://www.wowhead.com/classic/quest=5848/of-love-and-family
                name = "Of Love and Family",
                questId = 5848,
                level = 60,
                side = "Both",
                prereq = true,
                -- https://www.wowhead.com/classic/npc=11936/artist-renfray
                startNpc = "Artist Renfray", startNpcId = 11936, startLoc = "Western Plaguelands, Caer Darrow", startCoords = "65.8, 75.4",
                money = 39600,
            },
            {
                -- https://www.wowhead.com/classic/quest=5251/the-archivist
                name = "The Archivist",
                questId = 5251,
                level = 60,
                side = "Both",
                prereq = false,
                -- https://www.wowhead.com/classic/npc=11039/duke-nicholas-zverenhoff
                startNpc = "Duke Nicholas Zverenhoff", startNpcId = 11039, startLoc = "Eastern Plaguelands, Light's Hope Chapel", startCoords = "81.4, 59.8",
                money = 18000,
            },
            {
                -- https://www.wowhead.com/classic/quest=5214/the-great-ezra-grimm
                name = "The Great Ezra Grimm",
                questId = 5214,
                level = 60,
                side = "Both",
                prereq = false,
                startNpc = "Smokey LaRue", startNpcId = 11033, startLoc = "Eastern Plaguelands, Light's Hope Chapel", startCoords = "80.6, 58.0",
                rewards = {
                    { itemId = 13171, name = "Smokey's Lighter" },
                },
            },
            {
                -- https://www.wowhead.com/classic/quest=5122/the-medallion-of-faith
                name = "The Medallion of Faith",
                questId = 5122,
                level = 60,
                side = "Both",
                prereq = false,
                -- Aurius is inside the dungeon at the chapel near the Undead wing entrance; no world coordinates
                startNpc = "Aurius", startNpcId = 10917, startLoc = "Stratholme, at the chapel near the entrance to the Undead wing",
            },
            {
                -- https://www.wowhead.com/classic/quest=5282/the-restless-souls
                name = "The Restless Souls",
                questId = 5282,
                level = 60,
                side = "Both",
                prereq = true,
                -- https://www.wowhead.com/classic/npc=11140/egan
                startNpc = "Egan", startNpcId = 11140, startLoc = "Eastern Plaguelands, Terrordale", startCoords = "14.5, 33.7",
                money = 18000,
                rewards = {
                    { itemId = 13315, name = "Testament of Hope" },
                },
            },
            {
                -- https://www.wowhead.com/classic/quest=5262/the-truth-comes-crashing-down
                name = "The Truth Comes Crashing Down",
                questId = 5262,
                level = 60,
                side = "Both",
                prereq = true,
                startNpc = "Head of Balnazzar", startItemId = 13250, startLoc = "Drops from Balnazzar in Stratholme Living",
                money = 49800,
            },
        },
    },
    {
        id = "stratholme_undead",
        name = "Stratholme: Undead",
        zone = "Eastern Plaguelands",
        endBoss = "Baron Rivendare",
        bossLevel = 62,
        levelMin = 58,
        levelMax = 60,
        warnings = {
            "Undead packs in Stratholme chain-pull easily - clear carefully and watch for patrols.",
            "[[Baroness Anastari]] casts Possess (mind control) on a player - damage them to 50% HP to break it. She also casts Silence.",
            "[[Maleki the Pallid]] casts Ice Tomb on the tank, freezing them and switching targets. Be ready to off-tank or kite until freed.",
            "[[Magistrate Barthilas]] casts Mighty Blow - tank against a wall to avoid being knocked into other packs.",
            "[[Ramstein the Gorger]] casts Knockout, dropping the tank's threat. Watch threat - the next-highest player will be targeted.",
            "After [[Ramstein the Gorger]] dies, a large wave of Mindless Undead spawns. AoE them down before the Black Guard Sentries emerge.",
            "The door to [[Baron Rivendare]]'s room closes on engage. Make sure everyone is inside before pulling.",
            "[[Baron Rivendare]] casts Mortal Strike on the tank and summons skeletons. His Unholy Aura damages all nearby players - ranged stay at max range.",
        },
        quests = {
            {
                -- https://www.wowhead.com/classic/quest=5263/above-and-beyond
                name = "Above and Beyond",
                questId = 5263,
                level = 60,
                side = "Both",
                prereq = true,
                -- https://www.wowhead.com/classic/npc=11039/duke-nicholas-zverenhoff
                startNpc = "Duke Nicholas Zverenhoff", startNpcId = 11039, startLoc = "Eastern Plaguelands, Light's Hope Chapel", startCoords = "81.4, 59.8",
                money = 49800,
            },
            {
                -- https://www.wowhead.com/classic/quest=8945/dead-mans-plea
                name = "Dead Man's Plea",
                questId = 8945,
                level = 60,
                side = "Both",
                prereq = true,
                -- https://www.wowhead.com/classic/npc=16016/anthion-harmon
                startNpc = "Anthion Harmon", startNpcId = 16016, startLoc = "Eastern Plaguelands, outside Stratholme main entrance (requires Extra-Dimensional Ghost Revealer)", startCoords = "30.9, 16.8",
                rewards = {
                    { itemId = 22137, name = "Ysida's Satchel" },
                },
            },
            {
                -- https://www.wowhead.com/classic/quest=5243/houses-of-the-holy
                name = "Houses of the Holy",
                questId = 5243,
                level = 60,
                side = "Both",
                prereq = false,
                -- https://www.wowhead.com/classic/npc=11036/leonid-barthalomew-the-revered
                startNpc = "Leonid Barthalomew the Revered", startNpcId = 11036, startLoc = "Eastern Plaguelands, Light's Hope Chapel", startCoords = "81.7, 57.8",
                money = 18000,
                rewards = {
                    { itemId = 13217, name = "Band of the Penitent" },
                    { itemId = 13216, name = "Crown of the Penitent" },
                    { itemId = 3928, name = "Superior Healing Potion" },
                    { itemId = 6149, name = "Greater Mana Potion" },
                },
            },
            {
                -- https://www.wowhead.com/classic/quest=5463/menethils-gift
                name = "Menethil's Gift",
                questId = 5463,
                level = 60,
                side = "Both",
                prereq = true,
                -- https://www.wowhead.com/classic/npc=11036/leonid-barthalomew-the-revered
                startNpc = "Leonid Barthalomew the Revered", startNpcId = 11036, startLoc = "Eastern Plaguelands, Light's Hope Chapel", startCoords = "81.7, 57.8",
                money = 39600,
            },
            {
                -- https://www.wowhead.com/classic/quest=6163/ramstein
                name = "Ramstein",
                questId = 6163,
                level = 60,
                side = "Horde",
                prereq = true,
                -- https://www.wowhead.com/classic/npc=11878/nathanos-blightcaller
                startNpc = "Nathanos Blightcaller", startNpcId = 11878, startLoc = "Eastern Plaguelands, The Marris Stead", startCoords = "26.5, 74.7",
                money = 18000,
                rewards = {
                    { itemId = 17001, name = "Elemental Circle" },
                    { itemId = 18022, name = "Royal Seal of Alexis" },
                },
            },
            {
                -- https://www.wowhead.com/classic/quest=5213/the-active-agent
                name = "The Active Agent",
                questId = 5213,
                level = 60,
                side = "Both",
                prereq = true,
                -- https://www.wowhead.com/classic/npc=11035/betina-bigglezink
                startNpc = "Betina Bigglezink", startNpcId = 11035, startLoc = "Eastern Plaguelands, Light's Hope Chapel", startCoords = "81.5, 59.7",
                money = 39600,
                rewards = {
                    { itemId = 19812, name = "Rune of the Dawn" },
                    { itemId = 13209, name = "Seal of the Dawn" },
                },
            },
            {
                -- https://www.wowhead.com/classic/quest=5212/the-flesh-does-not-lie
                name = "The Flesh Does Not Lie",
                questId = 5212,
                level = 60,
                side = "Both",
                prereq = false,
                -- https://www.wowhead.com/classic/npc=11035/betina-bigglezink
                startNpc = "Betina Bigglezink", startNpcId = 11035, startLoc = "Eastern Plaguelands, Light's Hope Chapel", startCoords = "81.5, 59.7",
                money = 18000,
            },
        },
    },
    {
        id = "scholomance",
        name = "Scholomance",
        zone = "Western Plaguelands",
        endBoss = "Darkmaster Gandling",
        bossLevel = 61,
        levelMin = 58,
        levelMax = 60,
        warnings = {
            "[[Kirtonos the Herald]] has a knockback and should be tanked in front of a wall.",
            "[[Jandice Barov]] summons illusions of herself, either burn the boss or handle the adds.",
            "[[Rattlegore]] hits very hard, has a knockback, and cleaves.",
            "[[Ras Frostwhisper]] casts Fear and Chill Nova - ranged stay at max range. Interrupt as much as possible.",
            "[[Doctor Theolen Krastinov]] enrages at 50% and hits very hard - save defensive cooldowns.",
            "[[The Ravenian]] has a tank knockback and cleave.",
            "[[Lord Alexei Barov]] is a very tough fight - his unholy aura deals constant AoE damage, and his Veil of Shadow reduces healing on the target by 75%.",
            "[[Lady Illucia Barov]] will mind control a random player, and has an AoE silence.",
            "[[Darkmaster Gandling]] teleports random players (including the tank or healer) into side rooms.",
        },
        quests = {
            {
                -- https://www.wowhead.com/classic/quest=5343/barov-family-fortune
                name = "Barov Family Fortune",
                questId = 5343,
                level = 60,
                side = "Alliance",
                prereq = false,
                -- https://www.wowhead.com/classic/npc=11023/weldon-barov
                startNpc = "Weldon Barov", startNpcId = 11023, startLoc = "Western Plaguelands, Chillwind Camp", startCoords = "43.5, 83.7",
                money = 18000,
            },
            {
                -- https://www.wowhead.com/classic/quest=5341/barov-family-fortune
                name = "Barov Family Fortune",
                questId = 5341,
                level = 60,
                side = "Horde",
                prereq = false,
                -- https://www.wowhead.com/classic/npc=11022/alexi-barov
                startNpc = "Alexi Barov", startNpcId = 11022, startLoc = "Tirisfal Glades, The Bulwark", startCoords = "83.1, 71.6",
                money = 18000,
            },
            {
                name = "Dawn's Gambit",
                questId = 4771,
                level = 60,
                side = "Both",
                prereq = true,
                startNpc = "Betina Bigglezink", startNpcId = 11035, startLoc = "Eastern Plaguelands, Light's Hope Chapel", startCoords = "81.5, 59.7",
                money = 27000,
                rewards = {
                    { itemId = 15854, name = "Dancing Sliver" },
                    { itemId = 15853, name = "Windreaper" },
                },
            },
            {
                -- https://www.wowhead.com/classic/quest=5382/doctor-theolen-krastinov-the-butcher
                name = "Doctor Theolen Krastinov, the Butcher",
                questId = 5382,
                level = 60,
                side = "Both",
                prereq = false,
                -- https://www.wowhead.com/classic/npc=11216/eva-sarkhoff
                startNpc = "Eva Sarkhoff", startNpcId = 11216, startLoc = "Western Plaguelands, Caer Darrow (ghost)", startCoords = "70.2, 73.7",
                money = 39600,
            },
            {
                -- https://www.wowhead.com/classic/quest=5384/kirtonos-the-herald
                name = "Kirtonos the Herald",
                questId = 5384,
                level = 60,
                side = "Both",
                prereq = true,
                -- https://www.wowhead.com/classic/npc=11216/eva-sarkhoff
                startNpc = "Eva Sarkhoff", startNpcId = 11216, startLoc = "Western Plaguelands, Caer Darrow (ghost)", startCoords = "70.2, 73.7",
                money = 49800,
                rewards = {
                    { itemId = 15806, name = "Mirah's Song" },
                    { itemId = 15805, name = "Penelope's Rose" },
                },
            },
            {
                -- https://www.wowhead.com/classic/quest=5515/krastinovs-bag-of-horrors
                name = "Krastinov's Bag of Horrors",
                questId = 5515,
                level = 60,
                side = "Both",
                --https://www.wowhead.com/classic/npc=11216/eva-sarkhoff
                prereq = true,
                startNpc = "Eva Sarkhoff", startNpcId = 11216, startLoc = "Western Plaguelands, Caer Darrow (ghost)", startCoords = "70.2, 73.7",
                money = 39600,
            },
            {
                -- https://www.wowhead.com/classic/quest=5529/plagued-hatchlings
                name = "Plagued Hatchlings",
                questId = 5529,
                level = 58,
                side = "Both",
                prereq = false,
                -- https://www.wowhead.com/classic/npc=11035/betina-bigglezink
                startNpc = "Betina Bigglezink", startNpcId = 11035, startLoc = "Eastern Plaguelands, Light's Hope Chapel", startCoords = "81.5, 59.7",
                money = 9000,
            },
            {
                --https://www.wowhead.com/classic/quest=5466/the-lich-ras-frostwhisper
                name = "The Lich, Ras Frostwhisper",
                questId = 5466,
                level = 60,
                side = "Both",
                prereq = true,
                -- https://www.wowhead.com/classic/npc=11286/magistrate-marduke
                startNpc = "Magistrate Marduke", startNpcId = 11286, startLoc = "Western Plaguelands, Caer Darrow (ghost, requires Spectral Essence)", startCoords = "70.6, 74.1",
                money = 59700,
                rewards = {
                    { itemId = 13986, name = "Crown of Caer Darrow" },
                    { itemId = 13984, name = "Darrowspike" },
                    { itemId = 13982, name = "Warblade of Caer Darrow" },
                    { itemId = 14002, name = "Darrowshire Strongguard" },
                },
            },
        },
    },
}
